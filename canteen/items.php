...
//items query selects all fields from items table, looking specifically each ID
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="column center">
                ...
                <?php
                //results of each item record is echoed out
            ...
            <div class="column center">
                ...
                <label for="item" class="para-text-signup">Click below to see the information about an item</label>
                <!--Drinks form-->
                <div>
                    <form action="items.php" name="items_form" id="items_form" method="get">
                        <select id="item" name="item">
                            <!--options-->
                            <?php
                            //prints Item out within the dropdown
        ...
        <div class="container">
            <div class="row items-icon">
                <div class="column">
                    <form action="items.php" method="post" id="RefreshForm">
        ...
        <div id="items-content">
            <table id="itemstable" width="50%" class="table-striped table-center">
                <?php
                //if item-type is equal to savoury, all items listed as 'savoury' will be printed out within table
                if (isset($_POST['savoury'])) {
                    //ItemType query to populate savoury items table
        ...
        <div id="items-content">
            <table id="itemstable" width="50%" class="table-striped table-center">
                <?php
                // if item-type is equal to sweet, all items listed as 'sweet' will be printed out within table
                if (isset($_POST['sweet'])) {
                    //ItemType query to populate sweet items table
        ...
        <div id="items-content">
            <table id="itemstable" width="50%" class="table-striped table-center">
                <?php
                // if item-type is equal to cold, all items listed as 'cold' will be printed out within table
                if (isset($_POST['cold'])) {
                    //ItemType query to populate cold drinks table
        ...
        <div id="items-content">
            <table id="itemstable" width="50%" class="table-striped table-center">
                <?php
                // if item-type is equal to hot, all items listed as 'hot' will be printed out within table
                if (isset($_POST['hot'])) {
                    //ItemType query to populate hot drinks table
        ...
        <div class="row">
            <div class="column center">
                ...
                <hr style="border: 2px solid #89ADAE; width:10%">
                <label for="search" class="para-text-signup">Enter a key word to see if we have it on the menu!</label>
                <div class="center input-align">
                    <form action="items.php" method="post">
                        <input type="text" name="search" placeholder="Item Name" style="margin-right: 15rem;" id="search">

                        <br>
                        <?php
                        // function will use wildcards to search for specific key words in the menu
                        if (isset($_POST['search'])) {
                            ...
                            if ($search == "") {
                                ...
                                //search query to look for words in the items table similar to what the user types in
        ...
        <div id="items-content">
            <table id="itemstable" width="60%" class="table-striped table-center">
                <?php
                // if item is set to '1' available, all available items will be printed out in the table
                if (isset($_POST['available'])) {
                    //Availability query to populate available items table
        ...
        <div id="items-content">
            <table id="itemstable" width="60%" class="table-striped table-center">
                <?php
                // if item is set to '0' unavailable, all unavailable items will be printed out in the table
                if (isset($_POST['unavailable'])) {
                    //Availability query to populate unavailable items table
        ...
        <br>
        <h3>FULL MENU</h3>
        <p class="para-text-signup">Please click on the headings to see each field sorted</p>
        <?php


        $orderBy = !empty($_GET["orderby"]) ? $_GET["orderby"] : "Item"; #Stating what is ascending first. Item in this case is
        $order = !empty($_GET["order"]) ? $_GET["order"] : "asc"; #Order starts off as ascending

        //order by query orders items by what the variables $orderBy and $order state each item as (ASC)
        $order_query = "SELECT * FROM items ORDER BY " . $orderBy . " " . $order; #Orderby field chosen and start as ascending

        $result = mysqli_query($con, $order_query);

        //everything starts as ascending when the headers are clicked
        $ItemOrder = "asc";
        $CategoryOrder = "asc";
        $CaloriesOrder = "asc";
        $CostOrder = "asc";
        ...
        if ($orderBy == "Category" and $order == "asc") {
        ...
        if ($orderBy == "Calories" and $order == "asc") {
        ...
        ?>
        <div id="sort_content">
            <table width="100%" id="sort_table" class="table-striped">
                <tr class="row_header">
                    <th>
