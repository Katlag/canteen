...
if (empty(($Item) and ($ItemType) and ($Category) and ($Calories) and ($Cost) and ($Availability))) {
...
if (($Availability) != 1) {
    echo 'Availability must be set as 1 to start. Please try again. ';
    header("refresh:2; url = update_data.php");
    return $Availability;
}
...
if (ctype_alpha($ItemType) === false) {
    ...
    #All values are inserted as a new item within the items table
    ...
    #error check to see if item has successfully been inserted
